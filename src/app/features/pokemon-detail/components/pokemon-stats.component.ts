      ...
      >
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between border-b border-slate-700/50 pb-3">
            <span class="text-sm font-semibold tracking-wide text-slate-400 uppercase">Weight</span>
            <span
              class="rounded-lg border border-slate-700/30 bg-slate-900/50 px-3 py-1 font-medium text-white"
              >{{ pokemon().weight / 10 }} kg</span
            >
          </div>
          <div class="flex items-center justify-between border-b border-slate-700/50 pb-3">
            <span class="text-sm font-semibold tracking-wide text-slate-400 uppercase">Height</span>
            <span
              class="rounded-lg border border-slate-700/30 bg-slate-900/50 px-3 py-1 font-medium text-white"
          ...
          @for (stat of enrichedStats(); track stat.stat.name) {
            <div class="group space-y-1">
              <div class="flex justify-between text-sm">
                <span
                  class="font-medium text-slate-300 capitalize transition-colors group-hover:text-white"
                  >{{ stat.stat.name.replace('-', ' ') }}</span
                >
                <div class="flex gap-2">
                  @if (stat.effort > 0) {
                    <span
                      class="self-center rounded border border-amber-500/30 bg-amber-500/20 px-1.5 py-0.5 text-[10px] text-amber-300"
                      title="Effort Value"
                      >EV +{{ stat.effort }}</span
                    >
                  }
                  <span class="w-8 text-right font-bold text-white">{{ stat.base_stat }}</span>
                </div>
              </div>
              <div
                class="h-2.5 w-full overflow-hidden rounded-full border border-slate-700/50 bg-slate-900/80"
              >
                <div
